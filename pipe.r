...
compare_mc_partitioning = function(mc1_id, mc2_id, do_log=F, ord1='by_col', ord2='by_whichMax') 
{
	...
	csize = 2
	png(scfigs_fn(mc1_id, ofn_base), width=400 + ncol(tab_n) * csize * 2, height=200 + nrow(tab_n) * csize * 2)
	...
	if (!is.null(tt)) {
		png(scfigs_fn(mc1_id, paste0(ofn_base, "_collaped")), width=max(800, 400 + ncol(tt) * col_size) * 2, height=max(800, 200 + nrow(tt) * row_size) * 2)
...
generate_figs = function() 
{
	...
	for (qs in sort(unique(dysf_neto_strats))) {
		...
		png(scfigs_fn(tumor_t_nk_id, paste0("dysf_strat_de_by_prolif_", qs)), 200 * 2, (100 + 40 * 15) * 2)
		par(mar=c(4,8,4,1))
	...
	png(scfigs_fn(tumor_t_nk_id, "CD4_vs_CD8_by_panel", scfigs_dir(tumor_t_nk_id, "facs_idx")), w=900 * 2, h=300 * 2)
	par(mfrow=c(1,3))
	par(mar=c(3,3,3,3))
	...
	png(scfigs_fn(tumor_t_nk_id, "2d_proj_by_facs_type", scfigs_dir(tumor_t_nk_id, "facs_idx")), w=900 * 2, h=300 * 2)
	par(mfrow=c(1,3))
	par(mar=c(1,1,3,1))
